...
public class BasicNetwork implements Network {
            ...
            try {
               ...
               responseHeaders = convertHeaders(httpResponse.headers());
                ...
                if (httpResponse.body()!= null) {
                   ...
                   // System.out.println("tt1-" + new String(responseContents, "utf-8"));
    ...
    private void logSlowRequests(long requestLifetime, Request<?> request,
            byte[] responseContents, okhttp3.Response statusLine) {
        if (DEBUG || requestLifetime > SLOW_REQUEST_THRESHOLD_MS) {
            VolleyLog.d("HTTP response for request=<%s> [lifetime=%d], [size=%s], " +
                    "[rc=%d], [retryCount=%s]", request, requestLifetime,
                    responseContents != null ? responseContents.length : "null",
                    statusLine.code(), request.getRetryPolicy().getCurrentRetryCount());
        }
    }
